...
func CreateNewCluster(t *testing.T, clusterID uint64, servers []uint64) *ClusterSimulator {
	...
	for i := uint64(0); i < uint64(n); i++ {
		...
		for j := uint64(0); j < uint64(n); j++ {
			if serverInd == servers[j] {
				continue
			} else {
				peerList.Add(servers[j])
			}
		}

		var err error
		storage[serverInd], err = NewDatabase(uint64(clusterID), uint64(serverInd))
		if err != nil {
			t.Fatal(err)
		}
		commitChans[serverInd] = make(chan CommitEntry)
		serverList[serverInd] = CreateServer(serverInd, peerList, storage[serverInd], ready, commitChans[serverInd])

		serverList[serverInd].Serve()
		isAlive[serverInd] = true
	}
	...
	for i := uint64(0); i < uint64(n); i++ {
		...
		for j := uint64(0); j < uint64(n); j++ {
			...
			serverList[serverInd].ConnectToPeer(servers[j], serverList[servers[j]].GetListenerAddr())
		}
		isConnected[serverInd] = true
	}

	for i := uint64(0); i < uint64(n); i++ {
		activeServers.Add(servers[i])
	}
	close(ready)
	...
	for i := uint64(0); i < uint64(n); i++ {
		go newCluster.collectCommits(servers[i])
	}
	...
	for r := 0; r < 8; r++ {
		...
		for i := range nc.activeServers.peerSet {
			if nc.isConnected[i] {
				...
				if isLeader {
					if leaderId < 0 {
						leaderId = int(nc.raftCluster[i].serverId)
						leaderTerm = term
					} else {
						if nc.t != nil {
...
func (nc *ClusterSimulator) SubmitToServer(clusterID, serverId int, cmd interface{}) (bool, interface{}, error) {
	...
	case AddServers:
		...
		for i := uint64(0); i < uint64(len(serverIds)); i++ {
			...
			var err error
			nc.dbCluster[uint64(serverIds[i])], err = NewDatabase(uint64(clusterID), uint64(serverIds[i]))
			if err != nil {
