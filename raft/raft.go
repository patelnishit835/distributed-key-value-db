...
func (rn *RaftNode) persistToStorage() {
	// fmt.Println("Persisting to storage", rn.currentTerm, rn.votedFor, rn.log)
	for _, data := range []struct {
		name  string
		value interface{}
	}{{"currentTerm", rn.currentTerm}, {"votedFor", rn.votedFor}, {"log", rn.log}} {
		var buf bytes.Buffer
		// fmt.Println("Persisting to storage inside loop (name): ", data.name, "value: ", data.value)
		enc := gob.NewEncoder(&buf)
		if err := enc.Encode(data.value); err != nil {
			log.Fatal("encode error: ", err)
		}
		if err := rn.db.Set(data.name, buf.Bytes()); err != nil {
...
func (rn *RaftNode) restoreFromStorage() {
	for _, data := range []struct {
		...
		if value, found, err := rn.db.Get(data.name); err != nil {
...
func (rn *RaftNode) readFromStorage(key string, reply *string) error {
	if value, found, err := rn.db.Get(key); err != nil {
		return err
	} else if found {
		dec := gob.NewDecoder(bytes.NewBuffer(value))
		...
		return fmt.Errorf("KeyNotFound:%v", key)
	}
}
...
func (rn *RaftNode) Submit(command interface{}) (bool, interface{}, error) {
	...
	if rn.state == Leader {
		...
		case Read:
			...
			if readErr != nil {
				fmt.Println("Error in reading from storage: %v", readErr)
			}
			rn.mu.Unlock()
